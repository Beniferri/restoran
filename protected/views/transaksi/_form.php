<div class="row">
	<div class="col-sm-9">
		<div class="row">
			<div class="col-md-5 col-sm-6" id="list-item">
				<?php $this->widget('ext.bootstrap-select.TbSelect',array(
					'name'=>'scan',
					'data' => Produk::list_items('Pilih Produk'),
					'htmlOptions' => array(
			...
			<div class="col-md-7 col-sm-6 text-right">
				<button class="btn btn-default mar_top1" onclick="listItems();">Pencarian</button>
		...
		<div class="row">
			...
			<div class="col-md-6 col-sm-12 text-right" id="payment-button" style="display:<?php echo (Yii::app()->user->hasState('items_belanja') && count(Yii::app()->user->getState('items_belanja'))>0)? 'block':'none';?>">
				<?php echo CHtml::button('Pembayaran',array('onclick'=>'payRequest("'.Yii::app()->createUrl('transaksi/paymentRequest').'");','style'=>'min-width:100px;','id'=>'payment-btn','class'=>'btn btn-primary'));?>
				<?php echo CHtml::button('Batalkan Transaksi',array('onclick'=>'cancelRequest("'.Yii::app()->createUrl('transaksi/cancelTransaction').'");','style'=>'min-width:100px;','id'=>'cancel-btn','class'=>'btn btn-danger'));?>
	...
	<div class="col-sm-3">
		<div class="row">
			<div class="col-sm-12">
				<span class="fa fa-address-card"></span> <b><?php echo Yii::t('order','Pelanggan');?></b>
				<div class="mb10"></div>
				<?php $this->widget('ext.bootstrap-select.TbSelect',array(
					'name'=>'find_customer',
		...
		$.ajax({
			...
			'success': function(data){
				if(data.status=='success'){
					...
					$('#list-item').find(".filter-option").html('Pilih Produk');
				}else{
					alert(data.message);
					$("input[id=\'scan\']").val("");
					$("input[id=\'scan\']").focus();
		...
		$.ajax({
			...
			'success': function(data){
				if(data.status=='success'){
					...
					$('.modal-content .modal-title').html("<?php echo Yii::t('order','Daftar Produk');?>");
